...
中身はGoogleフォームをそのままページに埋め込んだものです。回答は自動的に、フォームに紐づくGoogleスプレッドシートへ集計されます（フォームの「回答」タブから確認できます）。

**フォームの内容を変更したい場合**: [フォームの編集画面](https://forms.gle/v8dnvnhX8GUVzTMP7) から質問項目を編集できます。フォームのURL自体を変更・再作成した場合は、`voice-form.html` 内の `<iframe>` の `src` と、その下のフォールバックリンクの2箇所を新しいURLに書き換えてください。
